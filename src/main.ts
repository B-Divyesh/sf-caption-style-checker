...
function profileNote() { const selected = PROFILES[profile] || PROFILES.youtube; return `<p class="profile-note">Rules reviewed ${selected.reviewed}. Review the final upload before publishing. <a href="${selected.source}" rel="external">Check ${esc(selected.label)} format guidance (external site).</a></p>`; }
function checker() {
